...
- 服务默认只监听 `127.0.0.1`，不要直接映射到公网或局域网；需要远程使用时，应放在带认证和 TLS 的反向代理后。
- 单次文本最多 20,000 个字符，合成并发和输出目录有硬上限；旧 MP3 会按限额清理。
- voice 只接受安全的服务标识格式，服务端会对 SSML 属性做 XML 转义；前端下拉框不是唯一校验。
- `start.bat` 会创建虚拟环境并安装依赖，首次安装应在不含生产凭据的隔离环境中执行，并审查 requirements 变更。
